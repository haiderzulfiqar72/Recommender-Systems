...
# for p in df['movieId'].unique():
#     c= predictionfunc(9,p) 

# num_sum= sim_ab*n_p[0] * (r_bp-r_b)*n_p[0]
# den_sum= n_p + sim_ab

# pred= r_a + (num_sum/den_sum)


# sim[sim['userId_x']==1 and [sim['userId_y']==2]]['sim']
# a+p

# c =df_join.groupby(['userId_x','userId_y']).agg(['mean','sum'])
# d= c.drop(['movieId'], axis=1)

# print(c.remove('movieID'))

# def sim(df, user1, user2):
   
#     filter1 = df["userId"]==1
#     df.where(filter1 & 'rating', inplace = True)

#     return sim
  
# sim(df, 1,2)

# df[df['userId']==1]['rating']
# # return df[df['userId']==user1]
# #sim= df.loc[(df['userId']==user1, columns = ['rating']) & df.loc[(df['userId']==user2, columns = ['rating'])]
# #data.where(filter1 & filter2, inplace = True)
# # df_join[(df_join['userId_x']==1) & (df_join['userId_y']==3)]

# def average_rating_user():
#         for (movieId, rating) in df.items():
#             avg_rating = df.groupby('userId').agg(mean_rating = ('rating', 'mean'))
#         return avg_rating
...
# #avg rating of each user a
# df.groupby('userId').agg(mean_rating = ('rating', 'mean'))

# avg_a= df.groupby('userId').agg(mean_rating = ('rating', 'mean'))
# # avg_b= df.groupby('userId_y').agg(mean_rating = ('rating_y', 'mean'))

# df_join[df_join['userId']==1]['rating'].agg('rating','mean')
# c=df.groupby('userId').agg('mean')


# pred_num= sum(sim)*(new['rating_y']-new['ry_mean'])
# pred_denom= sum(sim)

# pred= new['rx_mean'] + (pred_num/pred_denom)
